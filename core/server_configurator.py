...
def write_start_scripts(install_path: str, session: ServerSession,
                        jar_name: str = "server.jar", java_exe: str = "java") -> bool:
    """
    Write ``start.bat`` (Windows) and ``start.sh`` (Unix) launchers.

    ``java_exe`` is the Java command to invoke; when ServerCreator installed a
    bundled JDK this is its full path so the scripts also work outside the app.

    Returns ``True`` on success.
    """
    flags = jvm_flags(session)
    name = session.server_name
    java_bat = f'"{java_exe}"' if (" " in java_exe or "\\" in java_exe) else java_exe
    java_sh = f'"{java_exe}"' if (" " in java_exe or "/" in java_exe) else java_exe
    bat = (
        "@echo off\r\n"
        "chcp 65001 > nul\r\n"
        f"title {name} - Minecraft Server (ServerCreator)\r\n"
        f"echo [ServerCreator] \"{name}\" sunucusu baslatiliyor...\r\n"
        f"{java_bat} -Xms{session.xms_mb}M -Xmx{session.xmx_mb}M {flags} -jar {jar_name} nogui\r\n"
        "pause\r\n"
    )
    sh = (
        "#!/usr/bin/env bash\n"
        f"# Start script generated by ServerCreator for \"{name}\".\n"
        f"{java_sh} -Xms{session.xms_mb}M -Xmx{session.xmx_mb}M {flags} -jar {jar_name} nogui\n"
    )
    try:
